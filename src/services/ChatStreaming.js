...
const chat_api_key = apiKeyStore.getChatKey();
const translator_api_key = apiKeyStore.getTranslatorKey();
...
export async function* translatorStreaming(data) {
   // 1. 檢查 SSO token
  const userStore = useUserStore();
  const ok = await userStore.ensureValidToken();
  if (!ok) {
 ...
 const fetchUrl = "/dify//workflows/run";
  const response = await fetch(fetchUrl, {
    method: "POST",
    headers,
    body: JSON.stringify(data),
